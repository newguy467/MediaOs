...
-   **Stale version string** — `dashboard_widgets.py` referenced `2.0.20-dev` while the rest of the app used `2.0.27-dev`. Standardized to `next`.
-   **Missing `.gitignore`** — created comprehensive .gitignore to prevent committing `.env` secrets, databases, and build artifacts.

### Version

-   Bumped from `2.0.27-dev` → `next` across all files (VERSION, Dockerfile, version.py, main.py, dashboard\_widgets.py, plugins.py, example compose files, tests). The health endpoint now reports `version: next`.
