...
SETTINGS_GROUPS: dict[str, dict[str, tuple[type, str, bool]]] = {
    ...
    "system": {
        ...
        "ntfy_url": (str, "ntfy server base or full topic URL", False),
        "ntfy_topic": (str, "ntfy topic name", False),
        "ntfy_token": (str, "ntfy access token", True),
        "gotify_url": (str, "Gotify server URL", False),
        "gotify_token": (str, "Gotify app token", True),
        "webhook_url": (str, "Generic webhook URL (POSTs JSON)", False),
        "webhook_headers": (str, 'Extra webhook headers, JSON object e.g. {"Authorization": "Bearer xxx"}', True),
